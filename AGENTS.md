...
- **Primary keys are UUIDv7, generated by the application.** Every table uses `id uuid PRIMARY KEY` with no database default, and the value comes from `Uuid::now_v7()` at insert time. A sequence leaks the row count and lets anyone walk the table by guessing the next number; UUIDv7 keeps the index locality a sequence gave us, because its first 48 bits are a millisecond timestamp. A raw `INSERT` in a test or migration therefore has to supply the id itself.
...
- **Polling is what guarantees progress, not `LISTEN`.** The runner keeps the poll loop, the listen loop and the reaper as three independent tasks. A dropped listener connection reconnects and only makes the worker slower; it must never end a loop, or the process stays alive while quietly consuming nothing.
...
- The queue tests fail if a worker is running against the same database, because it eats the jobs they just published. Stop `just dev` before `just test` — killing the worker process alone is not enough, since `cargo-watch` restarts it. The claim assertions say so when they fail.
